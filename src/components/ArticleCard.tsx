...
export default function ArticleCard({ article, horizontal = false, featured = false }: ArticleCardProps) {
  ...
  if (featured) {
    return (
      <article className="col-span-full bg-white/[0.02] border border-white/[0.06] rounded-xl overflow-hidden group card-hover">
        <div className="flex flex-col sm:flex-row items-stretch">
          ...
          <div className="p-6 flex flex-col justify-center flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-3">
              <span className="text-[11px] font-medium px-2 py-0.5 rounded-full bg-indigo-500/15 text-indigo-400 flex items-center gap-1">
                <Tag size={10} />{article.category}
              </span>
              <span className="text-[11px] px-2 py-0.5 rounded-full bg-amber-500/15 text-amber-400 font-medium">精选</span>
            </div>
            <Link to={`/reviews/${slug}`}>
              <h2 className="text-lg font-bold text-slate-100 mb-2 group-hover:text-indigo-400 transition-colors line-clamp-2">{article.title}</h2>
            </Link>
  ...
  if (horizontal) {
    return (
      <article className="flex gap-4 group">
        <Link to={`/reviews/${slug}`} className="flex-shrink-0 w-24 h-16 md:w-32 md:h-20 rounded-lg overflow-hidden">
          {article.coverImage ? (
  ...
  // 默认卡片
  return (
    <article className="bg-white/[0.02] border border-white/[0.06] rounded-xl overflow-hidden group card-hover">
      <Link to={`/reviews/${slug}`} className="block overflow-hidden h-44 relative">
        {article.coverImage ? (
          <img src={article.coverImage} alt={article.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" />
        ) : (
          <div className="w-full h-full bg-gradient-to-br from-indigo-600/[0.08] to-cyan-600/[0.08]" />
      ...
      <div className="p-5">
        ...
        <Link to={`/reviews/${slug}`}>
          <h3 className="font-semibold text-slate-100 text-sm leading-snug mb-2 line-clamp-2 group-hover:text-indigo-400 transition-colors">
            {article.title}
          </h3>
        </Link>
        <p className="text-xs text-slate-500 line-clamp-2 mb-3 leading-relaxed">{article.excerpt}</p>
